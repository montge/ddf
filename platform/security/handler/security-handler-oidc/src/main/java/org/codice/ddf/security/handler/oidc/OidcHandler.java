...
public class OidcHandler implements AuthenticationHandler {
  ...
  private OidcCredentials getCredentialsFromRequest(OidcClient oidcClient, JEEContext jeeContext) {
    ...
    // In pac4j 5.x, use getCredentials() method instead of OidcCredentialsExtractor
    SessionStore sessionStore = new JEESessionStore();
    return (OidcCredentials) oidcClient.getCredentials(jeeContext, sessionStore).orElse(null);
  }
  ...
  private HandlerResult redirectForCredentials(
    ...
    Optional<RedirectionAction> redirectionAction =
        oidcClient.getRedirectionAction(jeeContext, sessionStore);
