/**
 * Copyright (c) Codice Foundation
 *
 * <p>This is free software: you can redistribute it and/or modify it under the terms of the GNU
 * Lesser General Public License as published by the Free Software Foundation, either version 3 of
 * the License, or any later version.
 *
 * <p>This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
 * without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Lesser General Public License for more details. A copy of the GNU Lesser General Public
 * License is distributed along with this program and can be found at
 * <http://www.gnu.org/licenses/lgpl.html>.
 */
package ddf.catalog.pubsub.predicate;

import static org.hamcrest.CoreMatchers.is;
import static org.hamcrest.MatcherAssert.assertThat;
import static org.junit.Assert.assertFalse;
import static org.junit.Assert.assertTrue;

import ddf.catalog.pubsub.internal.PubSubConstants;
import java.util.HashMap;
import org.junit.Test;
import org.osgi.service.event.Event;

public class SimplePredicatesTest {

  @Test
  public void testTruePredicateAlwaysReturnsTrue() {
    TruePredicate predicate = new TruePredicate();

    // Test with null event
    assertTrue(predicate.matches(null));

    // Test with empty properties
    Event emptyEvent = new Event("topic", new HashMap<>());
    assertTrue(predicate.matches(emptyEvent));

    // Test with populated properties
    HashMap<String, Object> properties = new HashMap<>();
    properties.put(PubSubConstants.HEADER_OPERATION_KEY, PubSubConstants.CREATE);
    properties.put(PubSubConstants.HEADER_ID_KEY, "test-id");
    Event populatedEvent = new Event("topic", properties);
    assertTrue(predicate.matches(populatedEvent));
  }

  @Test
  public void testFalsePredicateAlwaysReturnsFalse() {
    FalsePredicate predicate = new FalsePredicate();

    // Test with null event
    assertFalse(predicate.matches(null));

    // Test with empty properties
    Event emptyEvent = new Event("topic", new HashMap<>());
    assertFalse(predicate.matches(emptyEvent));

    // Test with populated properties
    HashMap<String, Object> properties = new HashMap<>();
    properties.put(PubSubConstants.HEADER_OPERATION_KEY, PubSubConstants.CREATE);
    properties.put(PubSubConstants.HEADER_ID_KEY, "test-id");
    Event populatedEvent = new Event("topic", properties);
    assertFalse(predicate.matches(populatedEvent));
  }

  @Test
  public void testTrueAndFalsePredicatesAreInverses() {
    TruePredicate truePredicate = new TruePredicate();
    FalsePredicate falsePredicate = new FalsePredicate();

    Event event = new Event("topic", new HashMap<>());

    // True and False should always be inverses
    assertThat(truePredicate.matches(event), is(!falsePredicate.matches(event)));
  }
}
