...
- [~] 5.4 SonarCloud measures — TOKEN FIXED 2026-06-10 (user regenerated; analysis + PR decoration now work; PR 210 gate served real findings). LAST BLOCKER for main-branch measures: the SonarCloud project's main branch is configured as `master` (never analysed) and the free plan discards non-main-branch (`main`) analyses. USER ACTION (1 min): sonarcloud.io → project montge_ddf → Administration → Branches & Pull Requests → rename main branch `master` → `main`, then re-run the SonarCloud Analysis workflow on main. Also pending there: review the 5 ReDoS hotspots (our pass-2 LINEARIZED regexes — mark "Safe") and consider a gate without the new-coverage condition (compile-only analyses always report 0% coverage)
...
- [x] 7.10 PR #210 MERGED to main 2026-06-10T22:58Z (merge commit 290d2a5b30, all required checks green; SonarCloud PR gate's code conditions clean: 0 bugs / 0 vulns / all ratings A after commit 58e7ade760 cleared 20 new-code findings + dropped Dual_EC_DRBG). develop fast-forwarded to main. Alert-count verification recorded in 5.1.
