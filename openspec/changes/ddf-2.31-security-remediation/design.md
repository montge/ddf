...
### Spring 6.2.11 — wrapped via Karaf `wrap:` (DECISION: build local wrapper) ✅ IMPLEMENTED
Bumping `${spring.version}` to 6.2.11 fixes CVE-2025-41249, but the deployed Karaf bundles were the ServiceMix wrappers pinned at `6.2.8_1`, and ServiceMix never published `6.2.11_1` (404). Spring 6.x jars ship **no OSGi manifest** of their own, so they can't be installed directly. **Implementation:** the kernel spring feature now wraps the official `org.springframework:*:6.2.11` jars via Karaf's `wrap:` protocol (already used in DDF for spring-oxm/spring-ldap), with `Export-Package=org.springframework.*;version=6.2.11` and `Import-Package=*;resolution:=optional`. Optional imports are safe here because a patch bump (6.2.8→6.2.11) adds no new required packages, and the DDF runtime already provides everything the 6.2.8 bundles used. All three spring properties moved to 6.2.11 in lockstep (no module pom depends on the ServiceMix coords — only the feature did). **Caveat:** build verifies compile + feature-XML structure only; OSGi resolution/binding needs validation on a running Karaf (blocked locally by the docs distribution issue). CVE-2025-41254 (STOMP CSRF) needs 6.2.12 — future.

### Shiro 1.13 → 2.1 (DECISION: DEFERRED to dedicated change)
Reversed from "in scope" after assessment: CVE severity is LOW while blast radius is high — 176 source files import `org.apache.shiro`; 2.x splits `shiro-core` into shiro-core/lang/crypto-*/config-* and requires multi-bundle feature re-wiring + moved-import fixes + full security regression. Mitigating facts (makes the eventual migration tractable): shiro-core 2.1.0 IS a proper OSGi bundle (`Bundle-SymbolicName: org.apache.shiro.core`), and `org.apache.shiro.util` (CollectionUtils, AntPathMatcher…) is retained in core. Tracked as task 4.1 for a focused effort with its own build/security-regression gate; PR #184 left open.
