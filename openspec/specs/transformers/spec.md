...
Specifies the pluggable transformer framework for metacard format conversion, including InputTransformer (file-to-metacard), MetacardTransformer (metacard-to-format), and QueryResponseTransformer (results-to-format) interfaces, along with built-in implementations for GeoJSON, XML, Atom, CSV, and document formats.
