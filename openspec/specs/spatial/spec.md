...
Specifies geospatial query support including WFS and CSW federation, OGC spatial query predicates (INTERSECTS, WITHIN, DWITHIN, etc.), the FilterAdapter architecture for converting OGC filters to provider-specific formats, distance unit normalization, and geometry format handling with WKT as the canonical format.
